            ...
            return (
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  ...
                  >
                    {delivered ? <CheckCircle size={13} /> : <Truck size={13} />}
                    {STATUS_LABELS[currentStatus]}
                  </span>
                </div>
                <p className="text-sm text-[#6b6b76]">
                  {o.items.length} Artikel · {o.total.toFixed(2)} € · Sendung {trackingNumber}
                </p>
                <p className="text-xs text-[#6b6b76]">
                  Aufgegeben am {o.placedAt.toLocaleDateString("de-DE")}
                  {!delivered && ` · Zustellung vsl. bis ${o.estDeliveryMax.toLocaleDateString("de-DE")}`}
                </p>
              </Link>
            );
          })}
